...
class Stamford:
    def __init__(self):
        ...
        f=open("dicts/words_obj.txt","r",encoding="utf-8")
        self.words_obj=[]
        self.syntax_error=[]
        self.words_replace=[]
        ...
        f=open("dicts/syntax_error.txt","r",encoding="utf-8")
        for line in f:
            line=line.split('\t')
            line[3]=line[3].strip()
            self.syntax_error.append(line)
        f.close()

        f=open("dicts/words_replace.txt","r",encoding="utf-8")
        for line in f:
            if '#' in line:
                continue
            line=line.split('\t')
        ...
        f=open("dicts/words_construction.txt","r",encoding="utf-8")
        for line in f:
            line=line.split('\t')
            line[4]=line[4].strip()
    ...
    def correction(self):
        for i in range(len(self.a)):
            for j in range(len(self.a[i])):
                for k in range(len(self.syntax_error)):
                     if self.syntax_error[k][0]==self.a[i][j][1]:
                         if self.a[i][j][int(self.syntax_error[k][1])]==self.syntax_error[k][2]:
                             self.a[i][j][int(self.syntax_error[k][1])]=self.syntax_error[k][3]
