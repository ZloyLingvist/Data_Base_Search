...
class Text_predicator:
        def __init__(self):
               ...
               f=open("dicts/obj.txt","r",encoding="utf-8")
               for line in f:
                       line=line.split('\t')
                       for x in line:
        ...
        def make_construction(self):
                       ...
                       for j in range(len(self.tmp[i])-1,-1,-1):
                               if self.tmp[i][j].split("::")[0] in self.obj_list:
                                       if j<len(self.tmp[i])-1:
                                               ...
                                               if "flat:foreign" in self.tmp[i][j+1]:
                ...
                for i in range(len(self.tmp)-1,-1,-1):
                        '''обозначение места объекта отправляем в конец, предварительно ищем конструкцию объект+обозначение'''
                        for j in range(len(self.tmp[i])-1,-1,-1):
                                if "flat:foreign" in self.tmp[i][j]:
                                        for p in range(len(self.tmp)):
                                                if len(self.tmp[p])==2:
                                                        if "obl" in self.tmp[p][0]:
                                                                temp=self.tmp[i][j]
                                                                '''если это предикат с exists'''
                                                                
                                                                if "exists" in self.tmp[i][0]:
                                                                        if "flat:foreign" in self.tmp[i][j-1]:
                                                                                self.tmp.append(['in',self.tmp[i][j],self.tmp[i][j-1]])
                                                                                del self.tmp[i][j-1]
                                                                                continue
        ...
        def make_predicate(self):
                ...
                for i in range(len(self.tmp)-1,-1,-1):
                        '''все, что содержит второй аргумент exists- в одну группу'''
                        if "exists" in self.tmp[i][0]:
                                temp=[]
